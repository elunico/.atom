...
(async function () {
  ...
  if (getCookie('hypixel-fetched') == 'true') {
    ...
    let objs = Object.values(products)
      .map(v => { return { 'id': v['product_id'], 'sell_price': v['quick_status']['sellPrice'] } });
    for (let obj of objs) {
    ...
    setCookie('hypixel-fetched', 'true', { expires: new Date(Date.now() + (1000 * 60 * 15))
        .toUTCString() });
